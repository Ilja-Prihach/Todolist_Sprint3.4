...
export const AppHttpRequests = () => {
  ...
  //         setTasks({ ...tasks, [todolist.id]: newTasks })
  //       })
  //     })
  //   })
  // }, [])
  ...
  const changeTaskStatus = (e: ChangeEvent<HTMLInputElement>, task: DomainTask) => {
    ...
    tasksApi.changeTask({ todolistId: task.todoListId, taskId: task.id, model }).then((res) => {
      const updatedTasks = res.data.data.item
      setTasks({
        ...tasks,
        [task.todoListId]: tasks[task.todoListId].map((el) => (el.id === task.id ? updatedTasks : el)),
      })
    })
  }
  ...
  const changeTaskTitle = (task: DomainTask, title: string) => {
    ...
    tasksApi.changeTask({ todolistId: task.todoListId, taskId: task.id, model }).then(() => {
      setTasks({
        ...tasks,
        [task.todoListId]: tasks[task.todoListId].map((el) => (el.id === task.id ? { ...el, title } : el)),
      })
    })
  }
